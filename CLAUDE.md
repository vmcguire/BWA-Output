# BWA-Output — Read this before doing anything
...
can clone, browse, and reason about BWA-Output on its own without
seeing the rest of the BWA monorepo.

All real work on BWA-Output happens inside the BWA monorepo at:

```
~/Developer/bwa-fx-ecosystem/plugins/output/
    dsp/           ← authoritative DSP implementation (THE source of truth)
    ui/            ← UI surface (Metal-rendered)
    plugin/        ← VST3 wrapper → builds BWA-Output.vst3
    manifest.json  ← name, controls, license tier, ecosystem benefits
```

If you've been asked to "work on BWA-Output," **navigate to the
...
1. **Standalone VST3** (this repo's release artifact) — `BWA-Output.vst3`
...
- **Category:** C (Spectral DSP Processor, Dual-Surface (post-merge))
- **Tech tree tier:** Tier 3
- **Role:** Final output trim (BWA-Mix master).
- **Pain attacked:** LUFS target output gain. Integrated meter. K-system display.
- **Scope verdict:** master-only
- **Band tier behavior:** Output is broadband; not band-tier-gated.

Full row in
[`MARKET-PAIN-AND-ATTACKS.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/MARKET-PAIN-AND-ATTACKS.md)
— search for `BWA-Output`.
...
4. **[`BWA-Architecture/docs/MARKET-PAIN-AND-ATTACKS.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/MARKET-PAIN-AND-ATTACKS.md)**
   — the row for BWA-Output. Internalize the pain + the attack before
...
- ❌ Committing to this repo. Go to the monorepo (`plugins/output/`).
...
- ❌ Adding new shared/* modules without checking if the capability folds
  ...
  Methodology Rule 3."
- ❌ Skipping the BWA-Output row in MARKET-PAIN-AND-ATTACKS.md. If the
  proposed work doesn't advance the BWA Attack for this plugin, push back.

## What "done" looks like

A `BWA-Output.vst3` that:
...
```bash
git subtree push --prefix=plugins/output \
  git@github.com:vmcguire/BWA-Output.git main
```

Then on GitHub:

1. Create a Release from `main` in this repo.
2. Attach the notarized `BWA-Output.vst3` zip.
3. Reference the monorepo commit SHA in the release notes.

---

_End of LLM onboarding for BWA-Output._
